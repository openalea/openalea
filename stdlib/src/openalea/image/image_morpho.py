...
__revision__ = " $Id: image_morpho.py 1861 2009-10-06 15:20:14Z cokelaer $ "

__all__ = ["surface", "barycenter", "pca", "extract_rasters",
           "connected_spaces", "set_labels"]
...
def extract_rasters (data_im) :
    ...
    return dict( (col, create_raster(pts)) \
                 for col, pts in raster_points.iteritems() )
            ...
            if data_im[i, j] > 0:
                ...
                if j > 0:
                    ...
                    if colj > 0:
                        ...
                        coli = label_im[i, j]
                        label_im[i, j] = colj
                        if coli > 0 and coli != colj:
                            #two different zones must be connected
                ...
                if new:#the point start a new zone
                    ...
                    free_label += 1
                points_list[label_im[i, j]].append( (i, j) )
                #add point to the list of point in this zone
    rasters = dict( (col, create_raster(pts)) \
                    for col, pts in points_list.iteritems() )
    return label_im, rasters

def set_labels (rasters, label_im, point_list) :
    return dict( (label,  rasters[label_im[x, y]]) \
                 for label, (x, y) in point_list.iteritems() )
