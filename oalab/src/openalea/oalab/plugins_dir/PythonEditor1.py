...
class PythonEditor1(CodeEditor, TextEditorOALab):
    __plugin_name__ = 'MonBeauEditeurPython'
    __categorie__ = 'TextEditor'

    
    def __init__(self, parent):
        super(PythonEditor1, self).__init__(parent)
        
    def setup(self):
        # Setup the editor
        super(PythonEditor1, self).setup_editor(go_to_definition=True) #C:\Spyder\spyder-2.1.11\spyderlib\widgets\sourcecode\codeeditor.py l.669
    ...
    def get_text(self, start='sof', end='eof'):
        ...
        return super(PythonEditor1, self).get_text(start, end)
        
    def set_text(self, text):
        # Set the text 'text' in the editor
        return super(PythonEditor1, self).set_text(text)
