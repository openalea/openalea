...
def ParseParameters(check=True):
    ...
    parser.add_option("", "--autosum",  metavar='AUTOSUM',
        default=None,
        help="autogenerate an autosum file from a filename provided as an argument")

    parser.add_option("", "--autosum-remove-namespace",  metavar='AUTOSUM',
        action="store_true",
        default=False,
        help="autogenerate an autosum file from a filename provided as an argument")
...
def autosum(opts):
    """ project must be openalea, vplants,alinea
    name is the package name
    """
    project = opts.project
    pkg_name = opts.package
    files = open(opts.autosum).readlines()


    print '.. _%s_reference:\n' % pkg_name
    print Tools.underline('Reference guide', '#')
    print '.. contents::\n'
    print Tools.underline('Class Inheritance diagram', '=')
    ...
    for mod in files:
        ...
        try:
            ...
            _doc = 'synopsis to be provided'

        # source = ../../src/
        if opts.autosum_remove_namespace:
            mod = module.replace('openalea.', '')
            mod = mod.replace('.','/') + '.py'
            pathtofile = os.path.join('../../src', mod)
        else:
        ...
        param = {
                ...
                'synopsis': _doc,
                'source': pathtofile,
                'module_title': Tools.underline (":mod:`%s` module" % module, '=')
                }
        print """
...
Download the source file :download:`%(source)s`.
