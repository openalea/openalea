...
class SpatialImageAnalysis(object):
    ...
    def surface_area(self, labels = None, walls = None):
        """
        Compute the surface area of cells with label in 'labels'. The wall area is the number of faces which are visible from the exterior of the cell

        :INPUT:
        Labels to compute

        :OUTPUT:
        Dictionary where the key is the label and the value is the wall area of this label
        """
        if labels == None:
            labels = self.labels()
        
        if walls == None:
            walls = self.wall_surfaces(real=False)
        A = dict( zip( labels, np.zeros_like(labels)))
        for key, value in walls.iteritems():
            if key[0] in labels:
                A[key[0]]+=value
            if key[1] in labels:
                A[key[1]]+=value            
    ...
    def compacity(self, labels = None, volumes = None, walls = None):
        """
        Compute the compacity of cells with label in 'labels' as described in 
        E. Bribiesca. An easy measure of compactness for 2d and 3d shapes. Pattern Recogn., 41(2):543–554, Feb. 2008.

        :INPUT:
        Labels to compute

        :OUTPUT:
        Dictionary where the key is the label and the value is the compacity of this label
        """
        if labels == None:
            labels = self.labels()
        
        if volumes == None:
            volumes = self.volume(labels = labels, real = False)

        data = dict( zip( labels, volumes ) )
        area = self.surface_area(labels = labels, walls = walls)
        C = {}
        for key in labels:
            n = data[key]
            C[key] = (n - area[key]/6) / (n - n**(2./3))
        return C

    
    def _calcul_cell_volume_convex_hull(self, sub_image, label):
        from openalea.plantgl.all import *
        f = Fit()
        points = np.transpose(np.where(sub_image==label))
        f.points = points.tolist()
        convex_hull = f.convexHull()
        return volume(convex_hull)
    
    
    def convex_hull_volume(self, labels = None, boundingboxes = None):
        """
        Compute the convex hull's volume of cells with labels in 'labels'

        :INPUT:
        labels to compute
        
        :OUPTUT:
        Dictionary where the key is the label and the value is the convex hull volume of this label
        """
        if labels == None:
            labels = self.labels()
            
        if boundingboxes == None:
            boundingboxes = self.boundingbox()
        data = {}
        bb = dict( zip( labels, boundingboxes ) )
        if 1 in labels:
            labels = labels[1:]
        for lab in labels:
            data[lab] = self._calcul_cell_volume_convex_hull(self.image[bb[lab]], lab)
        return data
