...
* TODO path.py: update to the latest version
* FIX change persistence.py name or move implementation to different files (io.py?)
* DONE: change RuntimeError to NotImplementedError
*FIX PyNodeFactoryWriter -> NodeFactory2Python adapter
* FIX Base class Reader & Writer are useless

* pkgmanager.py: get_wralea_[home, path]. Implement these functions based on a user config file.
* FIX Do not refer wralea name in the API. Use package instead.

* TODO all.py: use path.py rather than os.path

* FIX SubGraph.py: SubGraph -> CompositeNode
class CompositeNode( Graph, Node )
