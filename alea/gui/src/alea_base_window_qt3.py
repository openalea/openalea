...
# Form implementation generated from reading ui file 'alea_base_window_qt3.ui'
#
# Created: mer mar 8 17:14:58 2006
...
image4_data = \
    ...
    "\xa5\x49\x44\x41\x54\x38\x8d\xad\x52\xc1\x11\xc3" \
    "\x20\x0c\x93\xb9\x0e\x92\x87\xc7\xe2\xe1\x51\x18" \
    "\x85\x47\x46\xc9\xa3\x43\xc0\x2c\xee\xa3\x85\x33" \
    "\x10\x9a\xf4\xa8\x7e\x18\x49\xd6\x21\x80\x45\xd0" \
    "\x64\xae\x77\xb9\xfd\x50\x01\x40\x75\xd4\x13\x55" \
    "\x6a\xa3\xb1\x07\x2d\xc2\x9c\xf3\x24\x18\xc0\xcc" \
    "\x8d\xce\x01\x40\x4a\xa9\xae\xb4\xe2\x0f\x79\x40" \
    "\x8c\x7b\xe5\xbb\x42\x54\xd5\xaf\x9b\x8d\x18\x22" \
    "\xbe\x9e\xdd\xa5\x62\x6e\xa4\x4b\x06\xcb\x09\xfe" \
    "\x6e\x40\xa6\xe7\x29\x8e\xe3\x09\x11\x8f\x10\xc2" \
    "\xbd\x04\x29\xa5\x46\x5c\xb0\x6d\x0c\x11\x4f\x00" \
    "\xf0\x30\x7c\x62\x66\x05\xde\x55\xf5\x28\xd5\xf5" \
    "\x77\x43\xee\x18\x77\xb5\x3d\x9b\xb9\x35\x3b\xff" \
    "\xd7\xbd\xd1\x49\x8a\xeb\x87\xfa\x15\x2f\x98\x62" \
    "\x3e\x1b\xb7\xd6\x53\x49\x00\x00\x00\x00\x49\x45" \
    "\x4e\x44\xae\x42\x60\x82"
image5_data = \
    "\x89\x50\x4e\x47\x0d\x0a\x1a\x0a\x00\x00\x00\x0d" \
    "\x49\x48\x44\x52\x00\x00\x00\x16\x00\x00\x00\x16" \
...
class BaseWnd(QMainWindow):
    def __init__(self,parent = None,name = None,fl = 0):
        ...
        if not name:
            self.setName("BaseWnd")


        self.setCentralWidget(QWidget(self,"qt_central_widget"))
        BaseWndLayout = QHBoxLayout(self.centralWidget(),11,6,"BaseWndLayout")

        self.vsplitter = QSplitter(self.centralWidget(),"vsplitter")
        self.vsplitter.setOrientation(QSplitter.Horizontal)

        self.explore_frame = QTabWidget(self.vsplitter,"explore_frame")

        self.tab = QWidget(self.explore_frame,"tab")
        tabLayout = QVBoxLayout(self.tab,11,6,"tabLayout")
        self.explore_frame.insertTab(self.tab,QString(""))

        self.tab_2 = QWidget(self.explore_frame,"tab_2")
        tabLayout_2 = QVBoxLayout(self.tab_2,11,6,"tabLayout_2")
        self.explore_frame.insertTab(self.tab_2,QString(""))

        self.TabPage = QWidget(self.explore_frame,"TabPage")
        TabPageLayout = QVBoxLayout(self.TabPage,11,6,"TabPageLayout")
        self.explore_frame.insertTab(self.TabPage,QString(""))

        self.hsplitter = QSplitter(self.vsplitter,"hsplitter")
        self.hsplitter.setMinimumSize(QSize(99,73))
        self.hsplitter.setOrientation(QSplitter.Vertical)

        self.workspaces = QTabWidget(self.hsplitter,"workspaces")
        self.workspaces.setMinimumSize(QSize(0,0))

        self.tab_3 = QWidget(self.workspaces,"tab_3")
        self.workspaces.insertTab(self.tab_3,QString(""))

        self.shell_frame = QFrame(self.hsplitter,"shell_frame")
        self.shell_frame.setFrameShape(QFrame.StyledPanel)
        self.shell_frame.setFrameShadow(QFrame.Raised)
        BaseWndLayout.addWidget(self.vsplitter)
        ...
        self.new_itemAction = QAction(self,"new_itemAction")
        self.fileOpenHistoryAction = QAction(self,"fileOpenHistoryAction")
        self.fileSaveHistoryAction = QAction(self,"fileSaveHistoryAction")
        ...
        self.fileOpenHistoryAction.addTo(self.fileMenu)
        self.fileSaveAction.addTo(self.fileMenu)
        self.fileSaveHistoryAction.addTo(self.fileMenu)
        ...
        self.resize(QSize(769,663).expandedTo(self.minimumSizeHint()))
        ...
        self.connect(self.fileOpenHistoryAction,SIGNAL("activated()"),self.fileOpenHistory)
        self.connect(self.fileSaveHistoryAction,SIGNAL("activated()"),self.fileSaveHistory)


    def languageChange(self):
        self.setCaption(self.__tr("ALEA - Software for Plant Research"))
        self.explore_frame.changeTab(self.tab,self.__tr("Nodes"))
        self.explore_frame.changeTab(self.tab_2,self.__tr("Objects"))
        self.explore_frame.changeTab(self.TabPage,self.__tr("Applications"))
        self.workspaces.changeTab(self.tab_3,self.__tr("Workspace 1"))
        ...
        self.new_itemAction.setText(self.__tr("new item"))
        self.new_itemAction.setMenuText(self.__tr("new item"))
        self.fileOpenHistoryAction.setText(self.__tr("Open Python history"))
        self.fileOpenHistoryAction.setMenuText(self.__tr("Open &History"))
        self.fileSaveHistoryAction.setText(self.__tr("Save Python history"))
        self.fileSaveHistoryAction.setMenuText(self.__tr("Save H&istory"))
    ...
    def fileNew(self):
        print "BaseWnd.fileNew(): Not implemented yet"

    def fileOpen(self):
        print "BaseWnd.fileOpen(): Not implemented yet"

    def fileSave(self):
        print "BaseWnd.fileSave(): Not implemented yet"

    def fileSaveAs(self):
        print "BaseWnd.fileSaveAs(): Not implemented yet"

    def filePrint(self):
        print "BaseWnd.filePrint(): Not implemented yet"

    def fileExit(self):
        print "BaseWnd.fileExit(): Not implemented yet"

    def editUndo(self):
        print "BaseWnd.editUndo(): Not implemented yet"

    def editRedo(self):
        print "BaseWnd.editRedo(): Not implemented yet"

    def editCut(self):
        print "BaseWnd.editCut(): Not implemented yet"

    def editCopy(self):
        print "BaseWnd.editCopy(): Not implemented yet"

    def editPaste(self):
        print "BaseWnd.editPaste(): Not implemented yet"

    def editFind(self):
        print "BaseWnd.editFind(): Not implemented yet"

    def helpIndex(self):
        print "BaseWnd.helpIndex(): Not implemented yet"

    def helpContents(self):
        print "BaseWnd.helpContents(): Not implemented yet"

    def helpAbout(self):
        print "BaseWnd.helpAbout(): Not implemented yet"

    def fileOpenHistory(self):
        print "BaseWnd.fileOpenHistory(): Not implemented yet"

    def fileSaveHistory(self):
        print "BaseWnd.fileSaveHistory(): Not implemented yet"

    def __tr(self,s,c = None):
        return qApp.translate("BaseWnd",s,c)
